...
import {
  VoiceTranscript,
  SwipeReplay,
} from "@/components/admin/interviews/interview-turns";
...
// Deterministic pseudo-random seed → 60 bar heights
...
export function InterviewTranscriptDrawer({
  interviewId,
  open,
  onOpenChange,
}: Props) {
  const interview = interviewId
    ? adminInterviews.find((i) => i.id === interviewId)
    : null;
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-full overflow-y-auto bg-surface p-0 sm:max-w-[560px]"
      >
        {interview && (
