...
export function SourceOfFactChip({ fact, onActivate, active }: Props) {
  ...
  // Variant 1 — human-taught
  if (fact.factSource === "human") {
    const target: Activate | null = interview
      ? { kind: "interview", id: interview.id }
      : null;
    const isActive =
      active &&
      target &&
    ...
    return (
      ...
      >
        ...
        <span>
          {interview
            ? shortName(interview.interviewee.name)
            : fact.taughtBy ?? "human"}
        </span>
        {interview && (
          <>
  ...
  // Variant 2 — AI extraction with a linked interview that touched it
  if (interview) {
    const Icon = interview.mode === "voice" ? Mic : Layers;
    const target: Activate = { kind: "interview", id: interview.id };
    const isActive =
      active && active.kind === "interview" && active.id === interview.id;
    return (
      <button
        type="button"
  ...
  // Variant 3 — pure AI extraction from a source record
  const target: Activate = { kind: "source", id: fact.sourceId };
  const isActive =
    active && active.kind === "source" && active.id === fact.sourceId;
  return (
    <button
      type="button"
      onClick={() => onActivate(target)}
