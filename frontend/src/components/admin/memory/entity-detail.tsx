  ...
  return (
    <article className="rounded-[14px] border border-border bg-surface p-6 shadow-soft">
      ...
      <section className="mt-5">
        <div className="font-mono text-[10px] uppercase tracking-wider text-ink-soft">Notes</div>
      ...
      <section className="mt-6">
        <div className="mb-3 flex items-baseline justify-between">
          ...
          <div className="font-mono text-[10px] text-ink-soft">click a fact → trace its source</div>
        </div>
        {entityFacts.length === 0 ? (
          ...
          <ul className="space-y-2">
            {entityFacts.map((f) => (
              <FactRow key={f.id} fact={f} focus={focus} onFocusChange={onFocusChange} />
            ))}
          </ul>
        )}
      </section>
