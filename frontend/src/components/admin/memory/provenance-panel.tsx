...
import { adminInterviews, factInterviewIndex, type AdminInterview } from "@/lib/mock-data";
  ...
  return (
    <aside className="space-y-4">
      <section className="rounded-[14px] border border-border bg-surface p-4 shadow-soft">
        ...
        {backlinks.length === 0 ? (
          <p className="mt-2 text-[12px] italic text-ink-soft">Nothing links here yet.</p>
        ) : (
      ...
      <section className="rounded-[14px] border border-border bg-surface p-4 shadow-soft">
        ...
        <p className="mt-1.5 text-[11px] text-ink-soft">Every fact above traces to one of these.</p>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-wider text-ink-soft">
                      ...
                      <span className="ml-auto truncate normal-case lowercase">{s.id}</span>
                    </div>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="text-[12px] text-ink">{iv.interviewee.name}</div>
                        <div className="font-mono text-[9px] uppercase tracking-wider text-ink-soft">
                          {iv.id}
                        </div>
                      </div>
                      <span
                        ...
                        style={{
                          backgroundColor:
                            iv.mode === "voice" ? "var(--accent-soft)" : "var(--surface-2)",
                    ...
                    <div className="mt-1.5 text-[11px] text-ink-muted">{iv.topic}</div>
                    <button
                      type="button"
